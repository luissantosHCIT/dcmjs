...
function buildDefinedLengthSQBuffer(itemCodeValues, opts = {}) {
    ...
    // --- Meta header ---
    const metaBody = new WriteBufferStream({ defaultSize: 256, littleEndian: true});
    ...
    const seqBody = new WriteBufferStream({ defaultSize: 4096, littleEndian: true});
    for (const val of itemCodeValues) {
        // Write item content using the standard writer
        const itemBody = new WriteBufferStream({ defaultSize: 256, littleEndian: true});
        DicomMessage.writeTagObject(
            itemBody,
            "00080100",
            "SH",
    ...
    // --- Assemble Part 10 ---
    const file = new WriteBufferStream({ defaultSize: 8192, littleEndian: true});
